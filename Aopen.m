...
% mimu = load('E:\SCI\open code\ADIS16465.txt');
% GNSS_RTK = load('E:\SCI\open code\GNSS_RTK.txt');
load mimu_gnss1.mat;
truth = load('E:\SCI\open code\truth16465.txt');

%% Sensor data
ts = 1/200; % IMU Update Time
re = 6378137; % Long radius
Wie = 7.2921151467e-5; % Angular velocity of Earth's rotation
e = 1/298.257223563; % Eccentricity
b1 = 6356752.3142; % Short radius
e11 = sqrt(re^2-b1^2)/re; % First eccentricity
e12 = sqrt(re^2-b1^2)/b1; % Second eccentricity
c1 = re^2/b1;
Po = 180.*60.*60/pi;
g = [0;0;9.79361]; % Gravity vector
lb = [-0.073;0.302;0.087]; % Lever arm vector
...
%% Kalman filtering parameter setting
...
Q = diag([(30*(2.909*1e-4))^2 (30*(2.909*1e-4))^2 (30*(2.909*1e-4))^2 0 0 0]);
R_kf = diag([(3.*pi/180)^2 (3.*pi/180)^2 (3.*pi/180)^2]);
Xkf_attitude = [];
Xkf_gyroscope = [];
counter = 0;
...
for i = 2:hang1
    % upgrade Cbb
    % two sample
    Angle1 = [m_wx(i);m_wy(i);m_wz(i)]/2;
    Angle2 = [m_wx(i);m_wy(i);m_wz(i)]/2;
    A_b = Angle1 + Angle2 + cross(((2/3).*Angle1),Angle2);
...
%         IX_kf13I = norm(X_kf(1:3));
%         CNN = eye(3) - (sin(IX_kf13I)/IX_kf13I)*[0 -X_kf(3) X_kf(2);X_kf(3) 0 -X_kf(1);-X_kf(2) X_kf(1) 0] + ((1-cos(IX_kf13I))/(IX_kf13I)^2)*[0 -X_kf(3) X_kf(2);X_kf(3) 0 -X_kf(1);-X_kf(2) X_kf(1) 0]^2;
        ...
        f1pitch(i,1) = asin(-Cnb(3,1));
        f1roll(i,1) = atan(Cnb(3,2)/Cnb(3,3));
        if Cnb(3,3) < 0
            if f1roll(i,1) > 0
                f1roll(i,1) = f1roll(i,1) - pi;
            else
                f1roll(i,1) = f1roll(i,1) + pi;
            end
        end
        f1yaw(i,1) = atan(Cnb(2,1)/Cnb(1,1));
        if Cnb(2,1) > 0
            if abs(Cnb(1,1)) < 0.0001
                f1yaw(i,1) = pi/2;
            elseif Cnb(1,1) < 0
                f1yaw(i,1) = f1yaw(i,1) + pi;
            end
        else
            if abs(Cnb(1,1)) < 0.0001
                f1yaw(i,1) = -pi/2;
...
figure
subplot(311)
plot(Ti_mimu,f1roll.*(180/pi),'k',Ti_truth,truth_roll,'r');
title('Roll');
xlabel('Time(s)');
ylabel('Angle(deg)');
grid on;
legend('The proposed method','Truth')
subplot(312)
plot(Ti_mimu,f1pitch.*(180/pi),'k',Ti_truth,truth_pitch,'r');
title('Pitch');
xlabel('Time(s)');
ylabel('Angle(deg)');
grid on;
legend('The proposed method','Truth')
subplot(313)
plot(Ti_mimu,f1yaw.*(180/pi),'k',Ti_truth,truth_yaw,'r');
title('Yaw');
xlabel('Time(s)');
ylabel('Angle(deg)');
grid on;
